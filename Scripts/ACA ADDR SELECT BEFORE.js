...
try {
	...
	var multipleAddressSelected = false;

	if (typeof(AMENDADDRESS) == "object") {
		for (var j in AMENDADDRESS) {
			thisRow = AMENDADDRESS[j];
			if (thisRow["Change this Address"] == "Yes") {
				if (addressSelected) {
					multipleAddressSelected = true;
					break;
				} else {
		...
		if (multipleAddressSelected || !addressSelected) {
			cancel = true;
			showMessage = true;
		}

		if (multipleAddressSelected)
			comment("Please ensure only one address is selected from the list.");

		if (!addressSelected)
			comment("Must select at lesat one address to change.");
