...
try {
 ...
 if (typeof(COUNTYSUPPLYINGTO) == "object") 
 { 
	if (COUNTYSUPPLYINGTO) 
	{
		for (var i in  COUNTYSUPPLYINGTO) 
		{
			length = length + 1;
		}
		...
		{
 			var thisOuterRow =  COUNTYSUPPLYINGTO[j]["County"];
			if(thisOuterRow)
			{
				if (thisOuterRow.length() > 0) 
				{
					var k = parseInt(j)+ 1;
					for (; k< length; k++) 
					{
						
						var thisinnerRow =  COUNTYSUPPLYINGTO[k]["County"];
						if(thisinnerRow)
						{
							if (thisinnerRow.length() > 0) 
							{
								if(thisOuterRow == thisinnerRow)
								{
									errorMsg = "Same County can not be added more than once.";
									displayError = true;		
								}
							}
						}
 ...
 if (errorMsg && errorMsg.length > 0 && displayError) 
 {
  cancel = true;
  showMessage = true;
  message = "<br>Please correct the following issues:<br>" + errorMsg;
 }   
