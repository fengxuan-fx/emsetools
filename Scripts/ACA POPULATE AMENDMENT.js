...
try {
	...
	if (appMatch("Licenses/Plant/Commercial Fertilizer/License",parentCapId) || appMatch("Licenses/Plant/Ammonium Nitrate Fertilizer/License",parentCapId)) {
		
		var listOfFertilizer = new Array();
		loadASITables(parentCapId);
		
		if (appMatch("Licenses/Plant/Commercial Fertilizer/License",parentCapId)) {
			if (typeof(FERTILIZERBRANDPRODUCTNAMES) == "object") {
				for (ii in FERTILIZERBRANDPRODUCTNAMES) {
					var thisRow = FERTILIZERBRANDPRODUCTNAMES[ii];
					addRow(thisRow["Brand"]);
					logDebug("Brand Name table: added " + thisRow["Brand"]);
				}
			}
		}

		if (appMatch("Licenses/Plant/Ammonium Nitrate Fertilizer/License",parentCapId)) {
			if (typeof(PRODUCTBRAND) == "object") {
				for (jj in PRODUCTBRAND) {
					var thisRow = PRODUCTBRAND[jj];
					addRow(thisRow["Brand"]);
					logDebug("Brand Name table: added " + thisRow["Brand"]);
				}
			}
		...
		new_asit = addASITable4ACAPageFlow(asit,"BRAND LIST OF FERTILIZERS", listOfFertilizer);
