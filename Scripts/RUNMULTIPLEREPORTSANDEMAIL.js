...
function sendReport() {
	...
	try {
		...
		logDebug("Inside Send Report Function");
		
		for(var i=0; i < reportObj.length; i++){
		var thisReport = reportObj[i];
		var reportInfoResult = aa.reportManager.getReportInfoModelByName(thisReport.reportName);
		...
		report.setReportParameters(thisReport.reportParams);
