...
function sendReport() {
	logDebug("Inside Event Script");
	try {
		// Step 1.  Get Report Model by ReportName
		
		for(var i=0; i < reportObj.length; i++){
		
		thisReport = reportObj[i];
		var array = new Array();
		array = (thisReport.keySet()).toArray();
		var reportName = array[0];
		...
		report.setReportParameters(thisReport.get(array[0]));
