...
var bufferDays = aa.env.getValue("bufferDays");
...
if (!fromDate.length) // no "from" date, assume today + number of days to look ahead
	fromDate = dateAdd(null,parseInt(bufferDays))
	...
	/*
		//Create a capModel for use with the get records method
		 var emptyCm = aa.cap.getCapModel().getOutput();
			...
			emptyCt.setType("Milk Dealer");
			emptyCt.setSubType(appSubtype);
			emptyCt.setCategory("License");
			emptyCm.setCapType(emptyCt);
		
				emptyCm.getB1ExpirationModel().setExpStatus("Active");
				fromTestDate = new java.util.Date("12/1/2013");
				toTestDate = new java.util.Date("12/6/2013");
 
		var expResult = aa.cap.getCapsByAppTypeDateRange(emptyCm,fromTestDate,toTestDate);
	*/
...
/*
		if (newExpStatus.length > 0)
			...
			aa.print(altId + ": Update expiration status: " + newExpStatus);
			}
			else{
			logDebug("Do nothing");
			}
		...
		if (sendEmailToContactTypes.length > 0 && emailTemplate.length > 0) {
			...
			for (thisCon in conArray)
				...
				if (conEmail) {
					...
					addParameter(emailParameters,"$$licenseHolderName$$",b3Contact["firstName"] + " " + b3Contact["lastName"]);
					addParameter(emailParameters,"$$expirationDate$$",b1ExpDate);

					var capId4Email = aa.cap.createCapIDScriptModel(capId.ID1,capId.ID2,capId.ID3);

					var fileNames = [];

					

					result = aa.document.sendEmailAndSaveAsDocument(mailFrom,conEmail,"" , emailTemplate, emailParameters, capId4Email, null);
