...
function mainProcess()
{
	...
	setID1=  "Tonnage Letter" + "_" + dateString + "_" + "Amm Nitrate-First Notice";
	setID2= "Tonnage Letter" + "_" + dateString + "_" + "Amm Nitrate-Second Notice";
	...
	for (thisCap in vCapList)
	{
		...
		var statusCap = aa.cap.getCap(capId).getOutput().getCapStatus();
		...
		if(statusCap == "Active" || statusCap == "About to Expire" || statusCap == "Expired" || statusCap == "Tonnage Report Overdue" || statusCap == "Under Review - SAPA Applies"){
		var parentEstID = getEstParent("Establishment", capId);
		var array = new Array();
		array = getChildren("Licenses/Plant/Commercial Fertilizer/License", parent);
		for(zz in array){
		var status = aa.cap.getCap(array[zz]).getOutput().getCapStatus();
		...
		aa.print("Invalid Record");
		 ...
		 var capType = "Licenses/Amendment/Tonnage Report/Fertilizer";
		   ...
		   if(amendmentMonth == 10 && amendmentYear == year){
...
function runTestReport(reportName,itemCap)
{
