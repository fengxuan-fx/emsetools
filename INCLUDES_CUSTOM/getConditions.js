//overriding for DOS due to errors on the language section.
function getConditions(pType,pStatus,pDesc,pImpact) // optional capID
	...
	for (var thisCon in conArray)
		...
		//Check for related reference contacts too
		for (var thisCon in conArray) {
			if (conArray[thisCon].getCapContactModel().getPeople().getContactType() != "Applicant") continue; //only check for applicant
			if (conArray[thisCon].getCapContactModel().getPeople().contactSeqNumber) {
				var seqNumber4Lookup = conArray[thisCon].getCapContactModel().getPeople().contactSeqNumber;
				var thisConObj = getContactObjsBySeqNbr(itemCap,seqNumber4Lookup);

				if (thisConObj) {
					
					var relatedCons = thisConObj.getRelatedRefContacts(["Online Claimee"]);

					if (relatedCons) {
						for (var zz in relatedCons) {
							var conCondResult = aa.commonCondition.getCommonConditions("CONTACT", relatedCons[zz].getContactSeqNumber());
