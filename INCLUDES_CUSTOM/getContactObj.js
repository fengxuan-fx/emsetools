    ...
    if (capContactArray) {
        for (var yy in capContactArray) {
            if (capContactArray[yy].getPeople().contactType.toUpperCase().equals(typeToLoad.toUpperCase())) {
                ...
                return new contactObjTest(capContactArray[yy]);
            }
        }
    }
    ...
    if (ccsm)  {
        ...
        this.sendCreateAndLinkNotification = function() {
            ...
            if (toEmail) {
                ...
                if (this.refSeqNumber) {
                    var v = new verhoeff();
                    var pinCode = v.compute(this.refSeqNumber);
                    addParameter(params,"$$pinCode$$",pinCode);

                    sendNotification(sysFromEmail,toEmail,"","APPLICATION RECEIVED - CREATE AND LINK",params,null);                    
                }
